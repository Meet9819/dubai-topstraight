						...
						<div class="row">
							<div class="col-xs-12">
								<section class="panel">
                                    ...
                                    <?php
	$tempid=$_REQUEST["eid"];
	echo $result=mysql_query("select VIdeoName from videodata where VideoId=$tempid") or die("error");
	if(isset($result))
	{
...
if(isset($_REQUEST['SAVE'])) 
{
	...
	$ColumName= array('VIdeoName');
	$ColumValue= array($_REQUEST['pname'],$image_name);
	$objDB->Update('videodata','VideoId',$_REQUEST['pid'],$ColumName,$ColumValue);
	$objDB->close();
...
<?php
		...
		$tempid=$_REQUEST["eid"];
		$result=mysql_query("select * from videodata where VideoId=$tempid") or die("error");
		if(isset($result))
		{
			$tmp_val = mysql_fetch_row($result);
	  ?>
                                            ...
                                            <div class="form-group mb-lg">
								            <label class="col-md-2 control-label">Video Example</label>
                                            <div class="col-md-7">
								            http://www.youtube.com/watch?v=bfC4CpzBub4 From These Link Copy Only bfC4CpzBub4
                                            </div>
							                </div>
