...
class nicePaging{
	...
	public function pagerQuery($sql, $rowsPerPage)
	{
		...
		if($this->conn==null)
			...
			$result=mysql_query($sql, $this->conn) or die(header('location:index.php'));
	...
	public function createPaging($link)
	{
		...
		if($this->page>1)
		{
			$paging.='<a class="page-far-left" href="'.$link.$this->separator.'page=1" title="First page"></a>';
			$paging.='<a class="page-left" href="'.$link.$this->separator.'page='.($this->page-1).'" title="Previous page"></a>';
		}
		$paging.='<div id="page-info">';
		if($start>$this->maxPages){
			$paging.='<a class="" href="'.$link.$this->separator.'page='.($start-1).'" title="Page '.($start-1).'">...</a>';
		}
		for($i=$start;$i<=$end;$i++){
			if($this->page==$i){
				...
				$paging.='<a href="'.$link.$this->separator.'page='.$i.'" title="Page '.$i.'">'.$i.'</a>';
			}
		}
		if($end<$this->totalPages)
		{
			$paging.='<a href="'.$link.$this->separator.'page='.($end+1).'" title="Page '.($end+1).'">...</a>';
		}
		$paging.= '</div>';
		if($this->page<$this->totalPages){
			$paging.='<a href="'.$link.$this->separator.'page='.($this->page+1).'" title="Next page" class="page-right"></a>';
			$paging.='<a href="'.$link.$this->separator.'page='.$this->totalPages.'" title="Last page" class="page-far-right"></a>';
		}
