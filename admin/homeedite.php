...
{
	$ColumName= array('CmsName','CmsContent');
	$ColumValue= array($_REQUEST['pname'],$_REQUEST['description1']);
	$objDB->Update('cmsmain','CmsId',$_REQUEST['cid'],$ColumName,$ColumValue);
	$objDB->close();
...
?>
		...
		$tempid= $_REQUEST['eid'];
		$result=mysql_query("select * from cmsmain where CmsId=$tempid") or die("error");
		if(isset($result))
		{
	  ...
	  ?>
                            ...
                            <div class="form-group mb-lg">
								<label class="col-md-2 control-label">Menu Content</label>
								<div class="col-md-7">
								<?php
				  ...
				  $oFCKeditor->BasePath	= $sBasePath ;
				  		$result=mysql_query("select CmsContent from cmsmain where CmsId=$tempid");
						if(isset($result))
						{
							$DbValue = mysql_fetch_row($result);
							$oFCKeditor->Value   = $DbValue[0]; 
						}
