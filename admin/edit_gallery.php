						...
						<div class="row">
							<div class="col-xs-12">
								<section class="panel">
                                    ...
                                    <?php
	$tempid=$_REQUEST["eid"];
	echo $result=mysql_query("select image from tbl_product_img where productimg_id=$tempid") or die("error");
	if(isset($result))
	{
...
if(isset($_REQUEST['SAVE'])) 
{
	...
	$ColumName= array('productname','image');
	$ColumValue= array($_REQUEST['pname'],$image_name);
	$objDB->Update('tbl_product_img','productimg_id',$_REQUEST['pid'],$ColumName,$ColumValue);
	$objDB->close();
	print "<META http-equiv='refresh' content='0;URL=view_gallery.php'>";	
	exit;
}
?>
<?php
		...
		$tempid=$_REQUEST["eid"];
		$result=mysql_query("select * from tbl_product_img where productimg_id=$tempid") or die("error");
		if(isset($result))
		{
			$tmp_val = mysql_fetch_row($result);
	  ?>
							...
							<div class="form-group">
												<label class="col-md-2 control-label">Image</label>
												<div class="col-md-7">
												<img src="../images/<?php echo $tmp_val['1']; }?>" width="400" height="150" alt="No Image" />
												</div>
											</div>
