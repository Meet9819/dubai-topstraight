...
{
	$ColumName= array('category_name');
	$ColumValue= array($_REQUEST['pname']);
	$objDB->Update('tbl_category','category_id',$_REQUEST['cid'],$ColumName,$ColumValue);
	$objDB->close();
...
?>
		...
		$tempid= $_REQUEST['eid'];
		$result=mysql_query("select * from tbl_category where category_id=$tempid") or die("error");
		if(isset($result))
		{
