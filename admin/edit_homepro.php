						...
						<div class="row">
							<div class="col-xs-12">
								<section class="panel">
                                    ...
                                    <?php
	$tempid= $_REQUEST['eid'];
	echo $result=mysql_query("select * from tbl_homepro where homepro_id=$tempid") or die("error");
	if(isset($result))
	{
...
if(isset($_REQUEST['SAVE'])) 
{
	...
	$objDB->Update('tbl_homepro','homepro_id',$_REQUEST['pid'],$ColumName,$ColumValue);
	$objDB->close();
...
<?php
	  ...
	  ?>
                                            ...
                                            <div class="col-md-7">
								            <input name="pname" type="text" class="form-control input-lg" value="<?php  echo $tmp_val[2];  ?>" onKeyPress="return blockSpecialChar(event)" maxlength="40" required/>
                                            </div>
							                </div>
							...
							if(isset($result2))
							{
								...
								{
									if($tmp_val[1]==$tmpval['category_id'])
									{
										echo '<option selected="selected" value="'.$tmpval['category_id'].'"> '.$tmpval['category_name'].'</option>';
									}
									else
									{
