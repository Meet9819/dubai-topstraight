						...
						<div class="row">
							<div class="col-xs-12">
								<section class="panel">
                                    ...
                                    <?php
	$tempid= $_REQUEST['eid'];
	echo $result=mysql_query("select image from product_table where product_id=$tempid") or die("error");
	if(isset($result))
	{
...
?>
										<form  name="mainform" method="post" enctype="multipart/form-data" onSubmit="return checkfrm()">
<?php 
 ...
 if(isset($_REQUEST['SAVE'])) 
 {
	 ...
	 if($image1)
	 {
		...
		else
		{
			...
			{
				echo '<h1>The size of image is not more than 1 MB.!</h1>';
				$errors=1;
			}
			...
			{
				//pending delete
				if(strlen($oldimage)>5){unlink("../images/".$oldimage);}
			}
...
if(isset($_REQUEST['SAVE'])) 
{
	...
	$ColumName= array('homepro_id','product_name','product_price','prooffer','discription','image');
	$ColumValue= array($_REQUEST['pcat'],$_REQUEST['pname'],$_REQUEST['pgrade'],$_REQUEST['poffer'],$_REQUEST['pdescription'],$image_name);
	$objDB->Update('product_table','product_id',$_REQUEST['pid'],$ColumName,$ColumValue);
	$objDB->close();
...
?>
<?php
			  echo '<input type="hidden" name="pid" value="'.$_REQUEST['eid'].'" />';
		$tempid= $_REQUEST['eid'];
		$result=mysql_query("select * from product_table where product_id=$tempid") or die("error");
		if(isset($result))
		{
			$tmp_val = mysql_fetch_row($result);
	  ?>
                                            ...
                                            <div class="col-md-7">
								            <input name="pname" type="text" class="form-control input-lg" value="<?php  echo $tmp_val[2];  ?>"/>
                                            </div>
							                </div>
											<div class="form-group">
												...
												<div class="col-md-7">
													<select data-plugin-selectTwo class="form-control populate" name="pcat">
														<optgroup label="">
															 <?php
					  		$result2=mysql_query("select homepro_id,homepro_name from tbl_homepro");
							if(isset($result2))
							{
								while($tmpval = mysql_fetch_assoc($result2))
								{
									if($tmp_val[1]==$tmpval['homepro_id'])
									{
										echo '<option selected="selected" value="'.$tmpval['homepro_id'].'"> '.$tmpval['homepro_name'].'</option>';
									}
									else
									{
										echo '<option value="'.$tmpval['homepro_id'].'"> '.$tmpval['homepro_name'].'</option>';
									}
								}
							}
					?>
                                            ...
                                            <div class="form-group mb-lg">
								            <label class="col-md-2 control-label">Product Grade</label>
                                            <div class="col-md-7">
								            <input name="pgrade" type="text" class="form-control input-lg" value="<?php  echo $tmp_val[3];  ?>"/>
                                            </div>
							                </div>
						                    <div class="form-group">
						...
						$sBasePath = "../FCKeditor/"; //where the source files are locatedz
						$oFCKeditor = new FCKeditor('pdescription') ;
						$oFCKeditor->BasePath	= $sBasePath ;
						$oFCKeditor->Value   = $tmp_val[5]; 
  					...
  					?>
							...
							<div class="form-group">
												<label class="col-md-2 control-label">Image</label>
												<div class="col-md-7">
												<img src="../images/<?php echo $tmp_val['6']; }?>" width="400" height="150" alt="No Image" />
												</div>
											</div>
