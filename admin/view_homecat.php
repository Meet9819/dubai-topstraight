...
include("include/database.class.php");
		...
		$result=$paging->pagerQuery("SELECT * FROM tbl_homecat", $rowsPerPage);
		$total_id = '0';
		$tmpid=1;
		while($data=mysql_fetch_assoc($result))
		{
			...
			$xid=$data['homecat_id'];
		    echo   '<td>'.$count++.'</td>';
			echo    '<td>'.$data['homecat_name'].'</td>';
			//echo    '<td>';
			//echo    '<div class="col-sm-2 text-right">';
			//echo	'<a href="edit_homecat.php?eid='.$xid.'"><button type="update" class="btn btn-primary hidden-xs">Update</button></a>';
			//echo	'</div>';
           // echo    '<div class="col-sm-2 text-right">';
			//echo    '<a href="delete_homecat.php?eid='.$xid.'"><button type="delete" class="btn btn-primary hidden-xs">Delete</button></a>';
			//echo	'</div>';
			//echo	'</td>';
		    echo  	'</tr>';
			$tmpid++;
		...
		$link="view_homecat.php"; // Page name
