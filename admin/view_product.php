...
include("include/database.class.php");
		...
		$result=$paging->pagerQuery("SELECT * FROM product_table", $rowsPerPage);
		$total_id = '0';
		$tmpid=1;
		while($data=mysql_fetch_assoc($result))
		{
			...
			echo    '<td >'.$data['product_name'].'</td>';
				$tmpx=$data['homepro_id'];
				$result1=mysql_query("select homepro_name from tbl_homepro where homepro_id=$tmpx");
				if(isset($result1))
				{
			...
			echo    '<td align="left">'.$data['product_price'].'</td>';				
			echo    '<td align="left">'.$data['discription'].'</td>';
			echo    '<td><img src="../images/'.$data['image'].'" width="100" height="70" /></td>';
			
			
			echo    '<td>';
			echo    '<div class="col-sm-6 text-right">';
			echo	'<a href="edit_product.php?eid='.$xid.'"><button type="update" class="btn btn-primary hidden-xs">Update</button></a>';
			echo	'</div>';
            echo    '<div class="col-sm-2 text-right">';
			echo    '<a href="delete_product.php?eid='.$xid.'"><button type="delete" class="btn btn-primary hidden-xs">Delete</button></a>';
			echo	'</div>';
			echo	'</td>';
		    echo  	'</tr>';
			$tmpid++;
