...
include("include/database.class.php");
		...
		$result=$paging->pagerQuery("SELECT * FROM track", $rowsPerPage);
		$total_id = '0';
		$tmpid=1;
		while($data=mysql_fetch_assoc($result))
		{
			...
			echo    '<td>'.$data['rcvrname'].'</td>';
				echo    '<td>'.$data['web'].'</td>';
				echo    '<td>'.$data['forwn'].'</td>';
			
			echo    '<td>';
			echo    '<div class="col-sm-6 text-right">';
			echo	'<a href="track_edit.php?id='.$xid.'"><button type="update" class="btn btn-primary hidden-xs">Update</button></a>';
			echo	'</div>';
            echo    '<div class="col-sm-2 text-right">';
			echo    '<a href="track_delete?id='.$xid.'"><button type="delete" class="btn btn-primary hidden-xs">Delete</button></a>';
			echo	'</div>';
			echo	'</td>';
		    echo  	'</tr>';
			$tmpid++;
