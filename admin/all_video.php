...
include("include/database.class.php");
		...
		$result=$paging->pagerQuery("SELECT * FROM videodata", $rowsPerPage);
		$total_id = '0';
		$tmpid=1;
		while($data=mysql_fetch_assoc($result))
		{
			...
			$xid=$data['VideoId'];
			 echo   '<td>'.$count++.'</td>';
			echo 	'<td><iframe class="youtube-player" type="text/html" width="400" height="300" src="http://www.youtube.com/embed/'.$data['VIdeoName'].'" frameborder="0"></iframe></td>';
			...
			echo	'<a href="edit_video.php?eid='.$xid.'"><button type="update" class="btn btn-primary hidden-xs">Update</button></a>';
			echo	'</div>';
            echo    '<div class="col-sm-2 text-right">';
			echo    '<a href="delete_video.php?eid='.$xid.'"><button type="delete" class="btn btn-primary hidden-xs">Delete</button></a>';
			echo	'</div>';
			echo	'</td>';
		    echo  	'</tr>';
			$tmpid++;
